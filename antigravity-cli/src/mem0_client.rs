...
pub fn ensure_running() {
    ...
    if is_port_open(5001) {
        println!("[Antigravity CLI] Mem0 Docker service detected on port 5001.");
        return;
    }

    // Docker is offline, check Local Port
    if is_port_open(5000) {
        println!("[Antigravity CLI] Mem0 Local service detected on port 5000.");
        return;
    }

    // Local is offline, spawn it
    println!("[Antigravity CLI] Mem0 Docker (5001) and Local (5000) are offline. Spawning Mem0 Local...");
    ...
    match cmd.spawn() {
        Ok(_) => {
            println!("[Antigravity CLI] Successfully spawned Mem0 Local Service.");
            // Wait 500ms for initialization
            std::thread::sleep(Duration::from_millis(500));
        }
        Err(e) => {
            eprintln!("[Antigravity CLI] Error spawning Mem0 Local Service from {}: {}", bin_path.display(), e);
        }
    }
}
